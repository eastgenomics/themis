...
class JiraFunctions():
    ...
    def create_jira_info_dict(self, jira_api_response):
        ...
        -------
        jira_run_dict : dict
            nested dict where the summary name + assay_type is the main key.
        Example:
        {
        ...
        for issue in jira_api_response:
            ...
            # Check if the ticket name has the assay_type in it.
            match = re.match(regex_pattern, ticket_name)
            # If the match does not exist in ticket_name, log warning as the
            ...
            # If the assay type not in ticket_name, add it
            if not assay_type_in_name:
                ticket_name = f"{ticket_name}_{assay_type}"
