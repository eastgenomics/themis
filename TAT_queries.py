...
def login() -> None:
    ...
    try:
        ...
        print("Error logging in to DNAnexus")
        sys.exit(1)
...
def add_log_file_time(run_dict):
    ...
    for run_name in run_dict:
        ...
        run_dict[run_name]['log_file_created'] = (
            f"{time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(log_time))}"
        )
...
def find_earliest_002_job(run_dict):
    ...
    for run in run_dict:
        ...
        run_dict[run]['earliest_002_job'] = (
            f"{time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(first_job))}"
        )
    ...
    for job in find_jobs:
        ...
        if run_name in all_assays_dict:
            all_assays_dict[run_name]['demultiplex_started'] = (
                f"{time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(demulti_time))}"
            )
...
def create_all_assays_df(all_assays_dict):
    ...
    cols_to_convert = [
        ...
        'earliest_002_job', 'multiQC_finished'
    ]
...
def add_calculation_columns(all_assays_df):
    ...
    all_assays_df['log_file_to_first_002_job_days'] = (
